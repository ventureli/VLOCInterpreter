...
##变量类型
>具体内容请见VLOCLangugue 语法5全类型&全类型指针支持
>
##运算符
###普通运算符
...
*   "__VLOC_SETENV__" 设置环境变量 本次程序运行中可以在任意脚本中通过 "__VLOC_GETENV__" 读取
*  "__VLOC_GETENV__" 读取环境变量 (>=1.2)
* "__VLOC_SETUSERDEFAULT__" 设置userDefault 持久化，（当然也可以，自己用NSUserDefault 的代码写）(>=V1.2)
* "__VLOC_GETUSERDEFAULT__" 设置userDefault 持久化，（当然也可以，自己用NSUserDefault 的代码写）(>=v1.2)
* "__VLOC_FREE__" 强制释放变量
...
# VLOC language 语法4-OC类属性block等支持
##对OC支持
###OC 调用
